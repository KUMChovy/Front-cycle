...
export default function QuestionScreen({
    ...
    const decreaseAge = () => { if (age > 8) setAge((prev) => prev - 1); };
                ...
                <div className="w-full max-w-md sm:max-w-lg lg:max-w-xl">
                    ...
                    <header className="mb-10 flex items-center gap-3">
                        {current > 0 && (
                            <button
                                onClick={onBack}
                                aria-label="Volver"
                                className="mr-2 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 shadow-[0_6px_20px_rgba(0,0,0,0.12)] backdrop-blur transition-all duration-300 hover:bg-pink-100 hover:shadow-[0_10px_30px_rgba(251,113,133,0.35)] active:scale-[0.95] focus:outline-none focus:ring-4 focus:ring-pink-200"
                            >
                    ...
                    {question.type === "age" ? (
                        <div className="flex flex-col items-center gap-6">
                            <button onClick={increaseAge} className="text-6xl font-bold text-black">▲</button>
                            <div className="text-center">
                                <p className="text-xl text-black/40">{age - 1}</p>
                                <p className="text-4xl font-bold text-black">{age}</p>
                                <p className="text-xl text-black/40">{age + 1}</p>
                            </div>
                            <button onClick={decreaseAge} className="text-6xl font-bold text-black">▼</button>
                            <button
                                onClick={() => onAnswer(age)}
                                className="mt-6 rounded-full px-10 py-3 bg-white/90 text-black shadow-[0_6px_20px_rgba(0,0,0,0.08)] font-semibold"
                            >
                                Confirmar
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                            {question.options.map((option, index) => {
                                ...
                                // 🔧 Fix 1: comparar selected con value, no con option
                                const active = selected === value

                                return (
                                    <button
                                        // 🔧 Fix 2: key usa index siempre para evitar
                                        // colisiones cuando 2 opciones tienen el mismo id
                                        key={index}
                        ...
                        {Array.from({ length: total }).map((_, i) => (
                            <span
                                ...
                                className={`h-2 rounded-full transition-all ${
                                    i === current ? "w-6 bg-pink-400" : "w-2 bg-pink-200"
                                }`}
                            />
                        ))}
                    </div>
